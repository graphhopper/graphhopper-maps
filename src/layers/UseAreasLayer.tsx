...
function addAreasLayer(map: Map, cmEnabled: boolean, customModelStr: string) {
    ...
    if(map.getInteractions().getArray().some(i => i instanceof Draw)) return;
    ...
    source.on('addfeature', e => {
        ...
        if (e.feature) {
            ...
            if(customModel.areas?.features) {
                const numArr = customModel.areas.features.map((obj: any) => obj['id'] ? parseInt(obj['id'].match(/\d+/)[0]) : 0);
            ...
            areaFeature.geometry.coordinates[0] = areaFeature.geometry.coordinates[0].map((arr: number[]) =>
                [Math.round(arr[0] * 1_000_000) / 1_000_000, Math.round(arr[1] * 1_000_000) / 1_000_000])

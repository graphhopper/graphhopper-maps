...
export default function Search({points}: { points: QueryPoint[] }) {
    ...
    const [query, setQuery] = useState<Query>({
        point: {
            queryText: '', point: {lat: 0, lng: 0}, isInitialized: false, id: -1
        }, text: ''
    })
...
const GeocodingResults = ({hits, onSelectHit}: { hits: GeocodingHit[], onSelectHit: (hit: GeocodingHit) => void }) => {
    return (
        <div>
            <ul>
                {hits.map(hit => (<GeocodingEntry key={hit.osm_id} entry={hit} onSelectHit={onSelectHit}/>))}
            </ul>
        </div>)
}
...
function geocodingHitToAdress(hit: GeocodingHit) {
    ...
    if (hit.country)
        result = result ? result + ", " + hit.country : hit.country // if it is only the country, don't make a comma
    return result
}
...
const GeocodingEntry = ({entry, onSelectHit}: { entry: GeocodingHit, onSelectHit: (hit: GeocodingHit) => void }) => {
    return (
        <li>
            <button className={styles.selectableGeocodingEntry}

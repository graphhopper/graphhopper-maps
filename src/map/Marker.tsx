...
import { circleFontSize } from '@/layers/createMarkerSVG'
 ...
 */
export function CircleComponent({ color, number, size = 0, cursor = 'pointer' }: MarkerProps) {
    return (
        <svg
            aria-hidden="true"
            focusable="false"
 ...
 */
export function MarkerComponent({ color, number, size = 0, cursor = 'pointer' }: MarkerProps) {
    ...
    return (
        <svg
            ...
            viewBox="0 0 384 512"
            height={size}
            style={{ stroke: 'none', fill: color, cursor }}
        >
            <path d={MARKER_PATH} />
